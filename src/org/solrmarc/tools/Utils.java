...
//     * if "displayRawIfMissing" is a key in the map, then the raw field value is
//     * used. if "displayRawIfMissing" is not a key in the map, and the
//     * allowDefault param is set to true, then if the map contains "__DEFAULT"
//     * as a key, the value of "__DEFAULT" in the map is used; if allowDefault is
//     * true and there is neither "displayRawIfMissing" nor "__DEFAULT", as a key
//     * in the map, then if the map contains an empty key, the map value of the
//     * empty key is used. NOTE: If the spec for a field is supposed to contain
//     * all matching values, then the default lookup needs to be done here. If
//     * the spec for a field is only supposed to return the first matching
//     * mappable value, then the default impl should be done in the calling
//     * method
//     * 
//     * @param fieldVal
...
//     *            - the map to be used
//     * @param allowDefault
//     *            - if "displayRawIfMissing" is not a key in the map, and this
//     *            is to true, then if the map contains "__DEFAULT" as a key, the
//     *            value of "__DEFAULT" in the map is used.
...
//     * 
//     * @param set
//     *            the raw values to be mapped
//     * @param map
//     *            the map to be used
//     * @param allowDefault
//     *            if "displayRawIfMissing" is not a key in the map, and this is
//     *            to true, then if the map contains "__DEFAULT" as a key, the
//     *            value of "__DEFAULT" in the map is used.
//     * @return the new value, as determined by the impl.
//     */
...
//    }
    ...
    {
        locations = DataUtil.cleanData(locations);
    ...
    /**
     * returns <code>true</code> if the 3 letter language code is for a right to
     * left language (one written in arabic or hebrew characters)
     * 
     * @param langcode
    ...
    public final static boolean isRightToLeftLanguage(String langcode)
    {
        ...
        // hebrew characters
                langcode.equals("heb") || langcode.equals("yid") || langcode.equals("lad") || langcode.equals("jpr")
                || langcode.equals("jrb");
    }
     ...
     *         return the index of the first open paren; -1 if no unescaped open
     *         paren.
     */
    ...
    public static Set<String> returnValidISBNs(Set<String> candidates)
    {
        ...
        for (final String candidate : candidates)
        {
            ...
            if (p13.matcher(value).matches()) isbnSet.add(value.substring(0, 13));
     ...
     * the diacritic. Shamelessly stolen from UnicodeCharUtil class of
    ...
    public static void setLog4jLogLevel(org.apache.log4j.Level newLevel)
    {
         ...
         * and change their logging levels too. Also, skip sql loggers so they
         * do not get effected.
         */
