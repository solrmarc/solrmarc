...
public class IndexDriver extends BootableMain
{
    ...
    {
        logger = LogManager.getLogger(IndexDriver.class);
        this.args = args;
    }
    ...
    private void initializeFromOptions()
    {
        ...
        {
            logger.error("Error processing index configurations: " + specs + exceptions);
    ...
    final static String [] solrmarcPropertyStrings = {
            "org.solrmarc.indexer.chunksize",
            "org.solrmarc.indexer.buffersize",
            "org.solrmarc.indexer.threadcount",
            "org.solrmarc.solrj.threadcount",
            "org.solrmarc.track.solr.progress",
            "org.solrmarc.terminate.on.marc.exception",
            "org.solrmarc.output.redirect",
            "org.solrmarc.indexer.test.fire.method",
            "org.solrmarc.method.report",
    };
    ...
    private void configureReaderProps(String propertyFileURLStr) throws FileNotFoundException, IOException
    {
        ...
        if (propertyFileURLStr != null)
        {
            ...
            while (iter.hasMoreElements())
            {
                ...
                if (propertyName.startsWith("org.solrmarc.") && propertyStringsToCopy.contains(propertyName) && System.getProperty(propertyName) == null)
                {
    ...
    protected void processInput()
    {
        ...
        boolean perMethodReport = Boolean.parseBoolean(PropertyUtils.getProperty(readerProps, "org.solrmarc.method.report", "false"));
    ...
    private void handleRecordErrors()
    {
        ...
        for (final RecordAndDoc entry : errQ)
        {
            ...
            if (entry.errLocs.contains(RecordAndDoc.eErrorLocationVal.MARC_ERROR))
            {
                errTypeCnt[0][entry.getErrLvl().ordinal()]++;
            }
            if (entry.errLocs.contains(RecordAndDoc.eErrorLocationVal.INDEXING_ERROR))
            {
                errTypeCnt[1][entry.getErrLvl().ordinal()]++;
            }
            if (entry.errLocs.contains(RecordAndDoc.eErrorLocationVal.SOLR_ERROR))
            {
    ...
    protected void logTextForExceptions(List<IndexerSpecException> exceptions)
    {
        ...
        for (IndexerSpecException e : exceptions)
        {
            ...
            if (!specMessage.equals(lastSpec))
            {
                logger.debug(specMessage);
            }
            logger.debug(e.getMessage());
            for (Throwable cause = e.getCause(); cause != null; cause = cause.getCause())
            {
                logger.debug(e.getSolrField() + " : " + cause.getMessage());
            }
        }
    }
