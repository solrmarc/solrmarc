...
{
    private final static Logger logger = LogManager.getLogger(Indexer.class);
    private final static Logger dataExceptionlogger = LogManager.getLogger(SolrMarcDataException.class);
    ...
    protected RecordAndDoc indexToSolrDoc(final Record record)
    {
        ...
        for (final AbstractValueIndexer<?> indexer : indexers)
        {
            ...
            catch (InvocationTargetException ioe)
            {
                ...
                {
                    dataExceptionlogger.debug("Data Exception in record: " + recDoc.rec.getControlNumber());
                    dataExceptionlogger.debug(" while processing index specification: " + indexer.getSpecLabel());
                    if (wrapped != null)
                    {
                        dataExceptionlogger.debug(wrapped.getMessage());
                    }
                }
